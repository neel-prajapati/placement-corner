...
<script src="js/validation/formvalidationUpdateCompany.js"></script>
</head>
<%
...
<!--header start here-->
	...
	<div class="map"> <!-- ////////////////////// Main Division Starts ///////////////////// -->
		 <h1 style="font-family: 'poppinsbold' ,Arial, sans-serif;font-size: 40px;">Update Company</h1> 
...
<form action="CompanyController" method="post" id="company">
  ...
  <input type="hidden" value="<%=c.getCid()%>" name="id">
    ...
    <input type="text" class="form-control" id="cname" name="cname" value="<%=c.getName() %>" required>
  </div>
 
    <div class="form-group">
    <label for="pdate">Placement Date</label>
    <input type="date" id="datepicker" name="pdate" class="form-control" value="<%=c.getPlacement_date() %>" required="">
  </div>
  <div class="form-group">
    ...
    <label class="form-check-label" ><%=d.getDname() %></label>
  ...
  <div class="form-group">
    ...
    <input type="date" id="datepicker" name="sdate" class="form-control" value="<%=c.getReg_start_date() %>" required="">
  </div>
  <div class="form-group">
    <label for="edate">Registration End Date</label>
    <input type="date" id="datepicker" name="edate" class="form-control" value="<%=c.getReg_end_date() %>" required="">
  </div>
  <div class="form-group">
    <label for="status">Status</label>
    <input type="text" class="form-control" id="status"  name="status" value="<%=c.getStatus() %>" required>
  </div>
  <div class="form-group">
    <label for="location">Location</label>
    <input type="text" class="form-control" id="location"  name="location" value="<%=c.getLocation() %>" required>
  </div>
  <div class="form-group">
    <label for="stipend">Stipend</label>
    <input type="number" class="form-control" id="stipend"  name="stipend" value="<%=c.getStipend() %>" required>
  </div>
  <div class="form-group">
    <label for="ctc">CTC</label>
    <input type="number" class="form-control" id="ctc"  name="ctc" value="<%= c.getCtc()%>" required>
  </div>
  <div class="form-group">
    <label for="job">Job Profile</label>
    <input type="text" class="form-control" id="job"  name="job" value="<%= c.getJob()%>" required>
  </div>
  <div class="form-group">
    <label for="type">Type</label>
    <input type="text" class="form-control" id="type"  name="type" value="<%= c.getType()%>" required>
  </div>
  <div class="form-group">
    <label for="category">Category</label>
    <div>
					<select class="form-control" name="category" id="category"  name="category" required>
						<option value=" " disabled selected>Select Category</option>
						<%
						if(c.getCategory().equalsIgnoreCase("a"))
						{
							%>
						...
						%>
						<%
						if(c.getCategory().equalsIgnoreCase("a1"))
						{
							%>
  ...
  <div class="form-group">
    ...
    <input type="text"  class="form-control" id="cpi_ug"  name="cpi_ug" value="<%=c.getCpi_ug() %>" >
  </div>
  <div class="form-group">
    <label for="cpi_pg">PG CPI</label>
    <input type="text" class="form-control" id="cpi_pg" name="cpi_pg" value="<%= c.getCpi_pg()%>" >
  </div>
  <div class="form-group">
    <label for="description">Description</label>
     <textarea class="form-control" name="description" id="description" rows="5"  required> <%=c.getDescription() %></textarea>
  </div>
