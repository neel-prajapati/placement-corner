...
public class Logindao {
	...
	public void updatelogin(Loginbean s) {
		try {
			Connection conn = Projectutil.createConnection();
			String sql = "update  login set fname=?,mname=?,lname=?,dob=?,gender=?,height=?,blood_group=?,address=?,skypeid=?,email=?,mobile=?,did=?,year=? where adminid=?";
	...
	public void updateloginadmin(Loginbean s) {
		try {
			Connection conn = Projectutil.createConnection();
			String sql = "update  login set fname=?,mname=?,lname=?,dob=?,gender=?,address=?,email=?,mobile=?,did=? where id=?";
			PreparedStatement pst = conn.prepareStatement(sql);
			...
			pst.setString(8, s.getMobile());
			pst.setInt(9, s.getDid());
			pst.executeUpdate();
	...
	public  List<Loginbean> getallyears()
	{
		...
		{
			Connection conn=Projectutil.createConnection();
			String sql="SELECT  DISTINCT year FROM login  ";
			PreparedStatement pst=conn.prepareStatement(sql);
			ResultSet rs=pst.executeQuery();
	...
	public List<Loginbean> getAllloginsbyprogram(int did,int year)
	{
		List<Loginbean> list=new ArrayList<Loginbean>();
		try
		{
			Connection conn=Projectutil.createConnection();
			String sql="select * from login where did=? and year=?";
			PreparedStatement pst=conn.prepareStatement(sql);
			pst.setInt(1, did);
			pst.setInt(2, year);
			ResultSet rs=pst.executeQuery();
