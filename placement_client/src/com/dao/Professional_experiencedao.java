...
public class Professional_experiencedao {
	...
	public List<Professional_experiencebean> getAllexperiencesbyemailid(String email)
	{
		List<Professional_experiencebean> list=new ArrayList<Professional_experiencebean>();
		try
		{
			Connection conn=Projectutil.createConnection();
			String sql="select * from professional_experience where email=?";
			...
			while(rs.next())
			{
				Professional_experiencebean a=new Professional_experiencebean();
				a.setEmail(rs.getString("email"));
				a.setDescription(rs.getString("description"));
